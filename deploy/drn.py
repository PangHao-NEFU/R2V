...
class DRN(nn.Module):
    ...
    def __init__(
        ...
        if arch == 'C':
            self.conv1 = nn.Conv2d(
                3, channels[0], kernel_size=7, stride=1,
        ...
        self.layer5 = self._make_layer(
            block, channels[4], layers[4], dilation=2,
            new_level=False
        )
        ...
        if self.num_classes > 0:
            ...
            self.pred = nn.Conv2d(
                self.out_dim, num_classes, kernel_size=1,
    ...
    def _make_layer(
        ...
        for i in range(1, blocks):
            layers.append(
                block(
                    self.inplanes, planes, residual=residual,
    ...
    def _make_conv_layers(self, channels, convs, stride=1, dilation=1):
        ...
        for i in range(convs):
            modules.extend(
                [
                    nn.Conv2d(
                        self.inplanes, channels, kernel_size=3,
...
def drn_d_54(
    pretrained=False, out_map=256, num_classes=20, drn_model_dir='/serving/user_resource/checkpoint/',
    **kwargs
):
