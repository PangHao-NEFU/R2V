...
if TYPE_CHECKING:
    from OFA_OCR.fairseq.modules.multihead_attention import MultiheadAttention
...
def replace_unk(hypo_str, src_str, alignment, align_dict, unk):
    from OFA_OCR.fairseq import tokenizer
        ...
        if module_path not in import_user_module.memo:
            ...
            if module_name not in sys.modules:
                ...
                if os.path.exists(tasks_path):
                    from OFA_OCR.fairseq.tasks import import_tasks

                    import_tasks(tasks_path, f"{module_name}.tasks")

                models_path = os.path.join(module_path, "models")
                if os.path.exists(models_path):
                    from OFA_OCR.fairseq.models import import_models
...
def get_perplexity(loss, round=2, base=2):
    from OFA_OCR.fairseq.logging.meters import safe_round
...
def get_activation_fn(activation: str) -> Callable:
    ...
    from OFA_OCR.fairseq.modules import gelu, gelu_accurate
...
def tpu_data_loader(itr):
    ...
    from OFA_OCR.fairseq.data import iterators
