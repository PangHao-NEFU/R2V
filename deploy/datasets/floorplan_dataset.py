...
def augmentSample(options, image, background_colors=[], split='train'):
  ...
  if len(background_colors) == 0:
    ...
    full_image = background_colors[np.random.choice(np.arange(len(background_colors), dtype=np.int32),
...
class FloorplanDataset(Dataset):
  def __init__(self, options, split, dataFolder, random=True):
