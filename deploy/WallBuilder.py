...
class Builder(object):
    ...
    def build_floorplan_json(self, floor_plan_img_data, heat_map_img_list, measuring_scale_ratio=-1.0):
        ...
        Args:
            floor_plan_img_data: cv2读取的image npy
            heat_map_img_list: 模型预测的heatmap
            measuring_scale_ratio:
    ...
    def draw_points(
        self, wallPointsTemp, line_width=5, file_name="Node", background_img_data=None,
        rgb_color=[0, 0, 255]
    ):
    ...
    def _build_floorplan_primitives(self, floor_plan_img_data, heat_map_img_list):
        ...
        Args:
            floor_plan_img_data: 原图像
            heat_map_img_list: 网络预测的heatmap
        ...
        try:
            ...
            for i in range(13):
                ...
                junctions_list = self._extract_junctions(
                    cur_heat_map_img, 100, cur_category, cur_sub_category,
            ...
            self.all_opening_points = self._remove_duplicate_points(all_opening_points, is_window=True)
            ...
            self.all_opening_lines = self._build_opening_lines(self.all_opening_points, building_window=True)
            ...
            # #     np.uint8) * 255
            self.draw_lines(
                all_wall_lines=self.all_door_lines,
            ...
            self.draw_lines(
                all_wall_lines=self.all_opening_lines,
            ...
            #     np.uint8) * 255
            self.draw_lines(
                all_wall_lines=self.all_wall_lines,
            ...
            alignment_obj = WallAlignment(
                resize_floor_plan_img_data,
                self.all_wall_lines,
                self.all_wall_points,
                self.all_opening_points,
                self.all_door_points
        ...
        try:
            ...
            for wall_line in all_wall_lines:
                ...
                if line_dim == 0:
                    ...
                    cv2.putText(
                        image, (str(point_1.x) + ',' + str(point_1.y)), (point_1.x, point_1.y),
                    ...
                    cv2.putText(
                        image, (str(point_2.x) + ',' + str(point_2.y)), (point_2.x, point_2.y),
                        cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 255, 0)
                    )
                elif line_dim == -1:
                    if print_Point:
    ...
    def _repair_points_completeness(self):
        for tmp_wall_point in self.all_wall_points:
            ...
            for tmp_un_fit_dir in un_fit_directions:
                ...
                same_dir_points = self._find_points_on_same_direction(
                    self.all_wall_points, tmp_wall_point,
                    target_direction
                )
                ...
                # 是否被修复?
                self._repair_connect_points_completeness(
                    next_point, tmp_wall_point, next_index, index,
                    target_direction
                )
    ...
    def _remove_invalid_inclined_wall(self):
        ...
        inclined_wall_without_opening = self._remove_redundance_inclined_wall(
            inclined_wall_without_opening,
        ...
        if len(inclined_wall_without_opening) < 5:
            ...
            for j in range(i + 1, len(inclined_walls)):
                ...
                if i != j:
                    ...
                    if line_1.start_point.x < line_1.end_point.x:
                        line_1_slpoe, line_1_intercept = self._calculate_line_slope(
                    ...
                    if line_2.start_point.x < line_2.end_point.x:
                        line_2_slpoe, line_2_intercept = self._calculate_line_slope(
    ...
    def _add_misdeletion_inclined_wall(self, inclined_walls, inclined_wall_with_opening):
        ...
        point_map = {}
        for point in self.all_door_points:
    ...
    def _remove_redundance_inclined_wall(self, inclined_walls, inclined_wall_with_opening, not_inclined_wall):
        ...
        for i in range(len(inclined_walls)):
            ...
            if cur_wall.start_point.x == cur_wall.end_point.x:
                ...
                line_piexl = self._calculate_line_piexl(
            ...
            for point_info in line_piexl:
                ...
                cur_wall_hls_color = cv2.cvtColor(
                    np.array([b, g, r]).reshape((1, 1, 3)).astype(np.uint8),
                    cv2.COLOR_BGR2HLS
                )
                lr.append(r)
                lg.append(g)
    ...
    def _calc_wall_length(self):
        return np.sqrt(
            (self.start_point.x - self.end_point.x) * (self.start_point.x - self.end_point.x) +
    ...
    def _build_opening_lines(self, all_opening_points, building_door=False, building_window=False):
        try:
            ...
            while len(all_opening_points_copied) > 0:
                ...
                if building_door or building_window:
                    ...
                    for oblique_point in oblique_points:
                        ...
                        if building_window:
                            cur_opening_line = OpeningLine(cur_opening_point, oblique_point)
                        oblique_opening_lines.append(cur_opening_line)
                
                all_points = self._find_points_on_same_direction(
                    all_opening_points_copied, cur_opening_point,
                    direction
                )
                if len(all_points) == 0:
                    continue
            ...
            if building_door or building_window:
                ...
                for i in range(len(oblique_opening_lines)):
                    ...
                    for line in all_lines:
                        start_start_point_distance = self._get_point_distance(
                            line.start_point,
                ...
                for i in range(len(oblique_opening_lines)):
                    oblique_line = oblique_opening_lines[i]
                    for j in range(i + 1, len(oblique_opening_lines)):
                        oblique_line_2 = oblique_opening_lines[j]
                        inter_point = self._line_intersection(
                            oblique_line.start_point, oblique_line.end_point,
                            oblique_line_2.start_point, oblique_line_2.end_point
                        )
                        print(inter_point)
                        if len(inter_point) < 1:  # 只取没有相交的斜线
                            all_lines.append(oblique_line)
                            all_lines.append(oblique_line_2)
            return all_lines
        except Exception as err:
            print(err)
    ...
    def _line_intersection(self, start1, end1, start2, end2):
        ...
        if d != 0:
            ...
            return [int(lam * x1 + (1 - lam) * x2), int(lam * y1 + (1 - lam) * y2)]
    ...
    def _remove_duplicate_points(self, all_wall_points, is_wall=False, is_window=False):
        try:
            new_all_wall_points = []
            ...
            while len(all_wall_points) > 0:
                ...
                for tmp_point in all_wall_points:
                    ...
                    if dist < self.same_point_distance_threshold and (
                        ...
                        continue
                    inclined_dim = np.abs(
                        tmp_point.x - cur_wall_point.x
                    ) > self.same_point_distance_threshold and np.abs(
                        tmp_point.y - cur_wall_point.y
                    ) > self.same_point_distance_threshold
                    # 不对斜墙的点做去重，后面通过斜墙剪枝算法来剔除无效数据
                    if is_wall and inclined_dim and saved_point.type_category == 0:
                        continue
                    elif is_window and inclined_dim and dist < 50:
                        continue
                    else:
    ...
    def _is_infeasible_potential_wall_line_connection(
        self, point_1_forbidden_direction,
    ...
    def _extract_junctions(
        self, heat_map_img, max_target_points_number, junction_type_category,
        ...
        try:
            ...
            for pointIndex in range(max_target_points_number):
                ...
                if junction_class == 2:
                    cur_wall_junction = DoorPoint(
                        junction_type_category, junction_type_sub_category, x, y,
                        cur_max_value
                    )
                else:
                    cur_wall_junction = WallPoint(
                        junction_type_category, junction_type_sub_category, x, y,
                        cur_max_value
                    )
