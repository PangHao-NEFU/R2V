      ...
      for batchIndex in range(shape[0]):
        ...
        for i in range(cur_img_heatmaps.shape[2]):
          ...
          if i < 13:
            ...
            cur_heatmap_file_path = os.path.join(res_folder_path,
...
class WallBuilderDataDump(object):
  ...
  def _draw_wall_points(self, all_wall_points, line_width=8,
    ...
    if back_ground_img is None:
      img_mask = np.ones((self.wall_builder_obj.floor_plan_img_height, self.wall_builder_obj.floor_plan_img_width, 3),
                         np.uint8) * 0
    else:
  ...
  def _draw_wall_lines_range_box(self, wall_lines, back_ground_img=None, rgb_color=[255, 0, 0]):
    ...
    image = None
    if back_ground_img is None:
      image = np.ones((self.wall_builder_obj.floor_plan_img_height, self.wall_builder_obj.floor_plan_img_width, 3),
                      np.uint8) * 0
    else:
    ...
    for i in range(len(wall_lines)):
      ...
      if wall_line.line_dim() == -1:
        cv2.line(image, (wall_line.start_point.x, wall_line.start_point.y),
  ...
  def _draw_wall_lines(self, wall_lines, line_width=2, back_ground_img=None,
    ...
    for i in range(len(wall_lines)):
      ...
      if line_dim == 0:
        ...
        max(fixedValue - line_width, 0):min(fixedValue + line_width, self.wall_builder_obj.floor_plan_img_width),
        :] = draw_color
      else:
