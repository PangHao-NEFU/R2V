...
class DebugInfo(object):
    ...
    def save_corner_heatmaps_img(
        self, corner_tensor, img_transfor_obj=None, res_folder_path=r"./tmp", sample_index=0,
        corner_base_pred=None
        ):
        try:
            if not os.path.exists(res_folder_path):
                os.makedirs(res_folder_path)
            ...
            for batchIndex in range(shape[0]):
                ...
                for i in range(cur_img_heatmaps.shape[2]):
                    ...
                    if i < 13:
                        ...
                        cur_heatmap_file_path = os.path.join(
                            res_folder_path, "opening_heatmap_{0}.png".format(i + 1 - 13)
                            )
                        cur_heatmap *= 100
                    elif i < 8 + 13 + 28 + 4:
                        cur_heatmap_file_path = os.path.join(
                            res_folder_path, "door_heatmap_{0}.png".format(i + 1 - 13 - 8)
                            )
                        cur_heatmap *= 170
                    else:
                        cur_heatmap_file_path = os.path.join(
                            res_folder_path,
                            "icon_heatmap_{0}.png".format(i + 1 - 8 - 13 - 28 - 4)
                            )
                        cur_heatmap *= 255
...
class WallBuilderDataDump(object):
    ...
    def dump_data(self):
        ...
        self._draw_wall_lines(
            self.wall_builder_obj.all_wall_lines, line_width=2,
            back_ground_img=back_ground_img,
            rgb_color=[255, 0, 0]
            )
        # back_ground_img = self.wall_builder_obj.floor_plan_img_data.copy()
        self._draw_wall_lines_range_box(
            self.wall_builder_obj.all_wall_lines,
            back_ground_img=back_ground_img,
            rgb_color=[255, 0, 0]
            )
        # back_ground_img = self.wall_builder_obj.floor_plan_img_data.copy()
        self._draw_wall_lines(
            self.wall_builder_obj.all_opening_lines,
            back_ground_img=back_ground_img,
            rgb_color=[0, 255, 0]
            )
        
        back_ground_img = self.wall_builder_obj.floor_plan_img_data.copy()
        self._draw_door_lines(
            self.wall_builder_obj.all_door_lines,
            back_ground_img=back_ground_img
            )
        
        # back_ground_img = self.wall_builder_obj.floor_plan_img_data.copy()
        self._draw_wall_points(
            self.wall_builder_obj.all_wall_points, file_name="WallPoints.png",
            back_ground_img=back_ground_img,
            rgb_color=[0, 255, 0]
            )
        ...
        # back_ground_img = self.wall_builder_obj.floor_plan_img_data.copy()
        self._draw_wall_points(
            self.wall_builder_obj.all_opening_points, file_name="OpeningPoints.png",
            # line_width=2,
            back_ground_img=back_ground_img,
            rgb_color=[0, 0, 255]
            )
        ...
        back_ground_img = self.wall_builder_obj.floor_plan_img_data.copy()
        self._draw_wall_points(
            self.wall_builder_obj.all_door_points, file_name="DoorPoints.png",
            # line_width=2,
            back_ground_img=back_ground_img,
            rgb_color=[255, 0, 0]
            )
    ...
    def _draw_wall_points(
        self, all_wall_points, line_width=8,
    ...
    def _draw_wall_lines_range_box(self, wall_lines, back_ground_img=None, rgb_color=[255, 0, 0]):
        ...
        for i in range(len(wall_lines)):
            ...
            if wall_line.line_dim() == -1:
                cv2.line(
                    image, (wall_line.start_point.x, wall_line.start_point.y),
                    (wall_line.end_point.x, wall_line.end_point.y), line_color, 2
                    )
    
    def _draw_wall_lines(
        self, wall_lines, line_width=2, back_ground_img=None,
        ...
        for i in range(len(wall_lines)):
            ...
            if line_dim == 0:
                image[max(fixedValue - line_width, 0):min(
                    fixedValue + line_width, self.wall_builder_obj.floor_plan_img_height
                    ),
                ...
                max(fixedValue - line_width, 0):min(
                    fixedValue + line_width, self.wall_builder_obj.floor_plan_img_width
                    ),
                :] = draw_color
            else:
                cv2.line(
                    image, (point_1.x, point_1.y), (point_2.x, point_2.y),
                    (int(draw_color[1]), int(draw_color[2]), int(draw_color[0])), 2
                    )
            ...
            if line_dim == 0:
                ...
                if point_1.type_category < 3:
                    ...
                    if point_1.type_sub_category in [0, 7]:
                        ...
                        max(fixedValue, 0):min(fixedValue + 15, self.wall_builder_obj.floor_plan_img_width),
                        :] = line_color
                    else:
                        # 在左边
...
class WallBuilderFloorplanDataDump(WallBuilderDataDump):
    ...
    def _get_file_path(self, file_name):
        ...
        if completeness_flag and connectivity_status_flag and scale_status_flag and not pipeline_issue_flag:
            ...
            file_name_format = "Bad_{0}_{1}_{2}_{3}_{4}_{5}_{6}_{7}_{8}".format(
                "complete", completeness_flag,
