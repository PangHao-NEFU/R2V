    ...
    if objectType == 'icon':
      ...
      for indexGT, rectangleGT in enumerate(rectanglesGT):
        ...
        if len(matchedRectangleMask) == 0:
          print(indexGT, rectangleGT, [pointsGT[pointIndex][:2] for pointIndex in rectangleGT],
                'icon not found')
          pass
...
def reconstructFloorplan(wallCornerHeatmaps, doorCornerHeatmaps, iconCornerHeatmaps, iconHeatmaps, roomHeatmaps,
  ...
  try:
    ...
    for iconIndex in range(len(icons)):
      i_types.append(
        [model.addVar(vtype=GRB.BINARY, name="icon_type_" + str(iconIndex) + "_" + str(typeIndex)) for
         typeIndex in range(NUM_FINAL_ICONS)])
      continue
    ...
    if not gt:
      for wallIndex, wallLine in enumerate(wallLines):
        print(wallIndex,
              [np.array(wallPoints[pointIndex][:2]).astype(np.int32).tolist() for pointIndex in wallLine],
