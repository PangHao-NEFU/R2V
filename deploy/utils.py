...
NUM_CORNERS = 53
# CORNER_RANGES = {'wall': (0, 12), 'opening': (13, 16), 'single swing door': (17, 24), 'double swing door': (25, 32),
# 'DoorWindow': (33, 40), 'double slide Door': (41, 44), icon': (45, 48)}

NUM_WALL_CORNERS = 13  # 墙角点的种类有13种
NUM_OPENING_CORNERS = 4  # 开门点的种类有4种
NUM_BAY_WINDOW_CORNERS = 4  # 窗户的种类暂时4种

WALL_START_INDEX = 0
OPENING_START_INDEX = 13
BAY_WINDOW_START_INDEX = 17
DOOR_START_INDEX = 21
...
def cv2_read_image(file_path, type):
  try:
    if type == 'url':
      image = Image.open(requests.get(file_path, stream=True).raw)
      cv2_img = cv2.cvtColor(numpy.asarray(image), cv2.COLOR_RGB2BGR)
      return cv2_img
    else:
...
def cv2_write_image(img_data, file_path):
  if not cv2.imwrite(file_path, img_data):
    cv2.imencode('.jpg', img_data)[1].tofile(file_path)


def cv2_write_image_light(img_data, file_path):
  if not cv2.imwrite(file_path, img_data):
    cv2.imencode('.jpg', img_data)[1].tofile(file_path)
  ...
  for heatmapIndex in range(0, heatmaps.shape[-1]):
    ...
    for componentIndex in range(components.min() + 1, components.max() + 1):
      ...
      if ys.shape[0] <= numPixelsThreshold:
        continue
      # fpLog.info(heatmapIndex, xs.shape, ys.shape, componentIndex)
      if returnRanges:
...
def drawPoints(filename, width, height, points, backgroundImage=None, pointSize=5, pointColor=None):
  ...
  for point in points:
    if no_point_color:
      ...
      pass
    # fpLog.info('used', pointColor)
    # fpLog.info('color', point[2] , point[3])
