...
def augmentPoints(points, decreasingTypes=[2], increasingTypes=[1]):
  ...
  for pointIndex, point in enumerate(points):
    ...
    for orientation in range(4):
      ...
      newOrientations.append(orientation)
      newOrientations = tuple(newOrientations)
      if newOrientations not in orientationMap:
        continue
      newOrientation = orientationMap[newOrientations]
      newPoints.append([point[0], point[1], point[2] + 1, newOrientation])
      continue
...
def adjustPoints(points, lines):
  ...
  for lineIndex in range(len(lines)):
    ...
    xy = np.concatenate(
      [np.array([points[pointIndex][:2] for pointIndex in lines[lineIndex]]) for lineIndex in lineGroup],
      axis=0)
    mins = xy.min(0)
    maxs = xy.max(0)
...
def reconstructFloorplan(wallCornerHeatmaps, doorCornerHeatmaps, iconCornerHeatmaps, iconHeatmaps, roomHeatmaps,
                         output_prefix='test/', densityImage=None, gt_dict=None, gt=False, gap=-1, distanceThreshold=-1,
                         lengthThreshold=-1, debug_prefix='test', heatmapValueThresholdWall=None,
                         heatmapValueThresholdDoor=None, heatmapValueThresholdIcon=None, enableAugmentation=False):
  print('reconstruct')
  ...
  wallPoints, wallLines, wallPointOrientationLinesMap, wallPointNeighbors, augmentedPointMask = extractCorners(
    wallCornerHeatmaps, heatmapValueThresholdWall, gap=GAPS['wall_extraction'], augment=enableAugmentation,
    gt=gt)
  ...
  if True:
    ...
    for iconIndex in range(len(icons)):
      i_types.append(
        [LpVariable(cat=LpBinary, name="icon_type_" + str(iconIndex) + "_" + str(typeIndex)) for typeIndex
         in range(NUM_ICONS)])
      continue
    ...
    for lineIndex in range(len(wallLines)):
      ...
      for direction in range(2):
        ...
        for label in range(NUM_ROOMS):
          labels.append(LpVariable(cat=LpBinary,
    ...
    if False:
      ...
      for lineIndex in range(len(wallLines)):
        for oppositeRoomPair in oppositeRoomPairs:
          model += (l_dir_labels[lineIndex][0][oppositeRoomPair[0]] + l_dir_labels[lineIndex][0][
            oppositeRoomPair[1]] <= 1)
          if oppositeRoomPair[0] != oppositeRoomPair[1]:
            model += (l_dir_labels[lineIndex][0][oppositeRoomPair[1]] + l_dir_labels[lineIndex][0][
    ...
    for lineIndex in range(len(wallLines)):
      ...
      label = 0
      model += (l_dir_labels[lineIndex][0][label] + l_dir_labels[lineIndex][1][label] == w_l[lineIndex],
    ...
    for lineIndex, line in enumerate(wallLines):
      ...
      wallLineConfidence = np.sum(wallLineConfidenceMap[point_1[1]:point_2[1] + 1, point_1[0]:point_2[0] + 1]) / (
    ...
    for doorIndex, points in enumerate(doorWallPointMap):
      ...
      for pointIndex in points:
        model += (
          d_l[doorIndex] + w_p[pointIndex] <= 1,
          'door_on_two_walls_' + str(doorIndex) + '_' + str(pointIndex))
        continue
      continue
    ...
    if LpStatus[model.status] == 'Optimal':
      ...
      if not gt:
        ...
        adjustPoints(wallPoints, filteredWallLines)
        filteredWallLabels = [filteredWallLabels[lineIndex] for lineIndex in range(len(filteredWallLines)) if
      ...
      drawLines(output_prefix + 'result_line.png', width, height, wallPoints, filteredWallLines,
                filteredWallLabels, lineColor=255)
      ...
      drawRectangles(output_prefix + 'result_icon.png', width, height, iconPoints, filteredIcons,
                     filteredIconTypes)
